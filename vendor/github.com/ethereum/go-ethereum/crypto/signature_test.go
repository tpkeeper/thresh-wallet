...
func TestCreateAddress(t *testing.T) {
	//s:="0x027A6FD06C3825204FA57B7C7747E086CC5BEB3F51D7723B77C1EB9C2A70067175"
	//pubKey,_:=DecompressPubkey(hexutil.MustDecode(s))
	//addr:=PubkeyToAddress(*pubKey).String()
	//t.Log(addr)
	s:="0x66643033333730313031313462633130643565363038323331346239333035333966613135333965666261393164316632633332633430313034303336306563353330313238346464663336"
	bts:=hexutil.MustDecode(s)
	t.Log(string(bts))
	s1:="0x6331"
	bts1:=hexutil.MustDecode(s1)
	t.Log(string(bts1))

	s2:="0x66643033333730313031313462633130643565363038323331346239333035333966613135333965666261393164316632633332633430313034303332306161343430313263346464663336"
	bts2:=hexutil.MustDecode(s2)
	t.Log(string(bts2))
	s3:="0x66643033333730313031313462633130643565363038323331346239333035333966613135333965666261393164316632633332633430313034303336306563353330313263346464663336"
	bts3:=hexutil.MustDecode(s3)
	t.Log(string(bts3))

	s4:="0x66643033333730313031313435633634306635623734326135613131366130373437643364616630623339346630356464333934633430313034303336306563353330313262346464663336"
	bts4:=hexutil.MustDecode(s4)
	t.Log(string(bts4))

	s5:="0x66643033333730313031313435633634306635623734326135613131366130373437643364616630623339346630356464333934633430313034303332306131303730313031346464663336"
	bts5:=hexutil.MustDecode(s5)
	t.Log(string(bts5))

	s6:="0x6664303233373031303131343563363430663562373432613561313136613037343764336461663062333934663035646433393463343031303430323130323730313634346464663336"
	bts6:=hexutil.MustDecode(s6)
	t.Log(string(bts6))
}
